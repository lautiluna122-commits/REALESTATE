...
export function createAsset({ id, name, kind, path, projectId, isPrimary = false }) {
  return { id, name, kind, path, projectId, isPrimary, kindAsset: 'asset' };
}

export function createPlan({ id, name, path, unitId = null }) {
  return { id, name, path, unitId, kind: 'plan' };
}
...
export function createProjectConfig({ project = 'residential', building = 'tower', units = 'sales', amenities = ['pool', 'wellness'], environment = ['ocean', 'beach'], location = ['city', 'coast'], branding = {}, experience = { walking: true, floorSelection: true, apartmentTour: true, dayNight: true } }) {
...
export function createUnit({ id, floor, number, surface, bedrooms, bathrooms, terrace, price, currency = 'USD', status = UNIT_STATUS.AVAILABLE, description = '', plan = '', modelRef = '', images = [], projectId = '' }) {
  return { id, floor, number, surface, area: surface, bedrooms, bathrooms, terrace, price, currency, status, description, plan, modelRef, images, projectId, kind: 'unit' };
}
