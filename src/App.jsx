...
function ClientApp({ token }) {
  ...
  if(error)return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',padding:32,background:'#0b1214',color:'#fff'}}><section><p>REALESTATE · Cliente</p><h1>Enlace inválido</h1><p>{error}</p></section></main>;
  if(!project)return <RouteLoading label="Cargando espacio de la constructora…" />;
  return <Suspense fallback={<RouteLoading label="Cargando espacio de la constructora…" />}><AdminPortal sharedToken={token} sharedProject={project}/></Suspense>;
}
...
function PublicShowroom({ publicSlug }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/public/projects/${encodeURIComponent(publicSlug)}`)
      ...
      .then(payload => { if (cancelled) return; const hydrated=hydratePublishedProject(payload); if(!hydrated) throw new Error(); setProject(hydrated); setState('ready'); })
      .catch(() => { if(cancelled)return; if(localFallback){setProject(localFallback);setState('ready');}else{setState('error');setProject(null);} });
...
function Home() {
  return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',padding:32,background:'#0b1214',color:'#f5f1e9',fontFamily:'Arial,sans-serif'}}><section style={{maxWidth:760,width:'100%'}}><p style={{letterSpacing:3,opacity:.7}}>REALESTATE</p><h1 style={{fontSize:'clamp(42px,7vw,82px)',lineHeight:.95,margin:'18px 0'}}>Property,<br/><i>reimagined.</i></h1><p style={{fontSize:18,lineHeight:1.6,maxWidth:620,opacity:.8}}>Una infraestructura para que vos administres proyectos, las constructoras gestionen su contenido y cada comprador viva una experiencia digital independiente.</p><div style={{display:'flex',gap:12,flexWrap:'wrap',marginTop:28}}><a href="/proyecto/ocean-mansions" style={{padding:'14px 20px',background:'#f5f1e9',color:'#0b1214',textDecoration:'none'}}>Ver showroom demo ↗</a><a href="/admin" style={{padding:'14px 20px',border:'1px solid #536062',color:'#f5f1e9',textDecoration:'none'}}>Entrar a mi plataforma</a></div></section></main>;
}

export default function App() {
  const path = window.location.pathname;
  if (path === '/' || path === '') return <Home />;
  if (path === '/admin' || path.startsWith('/admin/')) return <Suspense fallback={<RouteLoading label="Cargando plataforma…" />}><AdminPortal platform /></Suspense>;
  if (path === '/platform' || path.startsWith('/platform/')) return <Suspense fallback={<RouteLoading label="Cargando plataforma…" />}><AdminPortal platform /></Suspense>;
  if (path === '/workspace' || path.startsWith('/workspace/')) return <Suspense fallback={<RouteLoading label="Cargando cliente…" />}><AdminPortal /></Suspense>;
  const clientMatch = path.match(/^\/cliente\/([^/]+)\/?$/); if (clientMatch) return <ClientApp token={clientMatch[1]} />;
  const match = path.match(/^\/proyecto\/([^/]+)\/?$/); if (!match) return <Home />;
