...
const clientUrl = `/cliente/${slug}`;
const readerUrl = `/embed/${slug}`;
...
const contentCards = [
  ...
  ['VID', 'Vídeos', 'MP4 / WEBM', 'Agregar vídeo', false], ['AM', 'Amenities', 'Imágenes + datos', 'Editar amenities', true],
];
...
export default function AdminDashboard() {
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar"><div className="admin-brand"><span>RE</span><div><strong>REAL ESTATE</strong><small>SHOWROOM STUDIO</small></div></div>
      <div className="admin-workspace"><small>PROYECTO ACTIVO</small><button>{project.name}<span>⌄</span></button></div>
    ...
    <main className="admin-main"><header className="admin-topbar"><div><span className="eyebrow">PROJECT STUDIO / {label(section)}</span><h1>{section === 'overview' ? 'Panel del proyecto' : label(section)}</h1></div><div className="top-actions"><span className="status-dot">Publicado</span><a className="preview-button" href={`/proyecto/${slug}`}>Vista previa ↗</a></div></header>
      {section === 'overview' && <Overview available={available.length} reserved={reserved.length} units={units.length} setSection={setSection} />}
      {section === 'content' && <Content />}{section === 'inventory' && <Inventory units={units} />}{section === 'experience' && <Experience setSection={setSection} />}
...
function Overview({ available, reserved, units, setSection }) { return <div className="dashboard-grid">
  ...
  <section className="section-card wide"><CardHead title="Centro de control" text="Prepará, revisá y publicá cada showroom desde un solo motor." /><div className="entry-grid">{[['content','Contenido','Renders, modelo 3D, interiores, vídeos y amenities.'],['inventory','Inventario','Unidades, precios, superficie y disponibilidad.'],['experience','Experiencia','Cámara, navegación, transiciones y recorrido.'],['plans','Planos','Planos por piso y vinculación de unidades.'],['branding','Branding','Logo, tipografía, colores y estilo visual.'],['publish','Publicar','Generá los accesos del cliente y del comprador.']].map(([id,t,d]) => <button className="entry" key={id} onClick={() => setSection(id)}><span>{icon(id)}</span><div><strong>{t}</strong><small>{d}</small></div><b>→</b></button>)}</div></section>
</div>; }
function Content() { return <div className="section-card"><CardHead title="Contenido del proyecto" text="Assets técnicos y comerciales que alimentan el motor del showroom." /><div className="dropzone"><div>＋</div><strong>Arrastrá renders, modelos, planos o vídeos</strong><span>JPG · PNG · WEBP · GLB · GLTF · MP4 · MOV · PDF</span><button>Seleccionar archivos</button></div><div className="content-grid">{contentCards.map(([tag,title,type,state,ready]) => <article className="asset-card" key={title}><div className="asset-thumb"><span>{tag}</span>{ready && <em>✓</em>}</div><div><strong>{title}</strong><small>{type}</small><span className={ready ? 'ready' : ''}>{state}</span></div></article>)}</div></div>; }
...
function Branding() { return <div className="section-card"><CardHead title="Branding" text="Identidad visual aplicada automáticamente al showroom."/><div className="branding-preview"><div className="brand-sample"><span>OM</span><h2>{project.name}</h2><p>Digital Property Experience</p></div><div className="brand-values"><div><small>PRIMARIO</small><b>#173B63</b></div><div><small>ACENTO</small><b>#D4AF69</b></div><div><small>TIPOGRAFÍA</small><b>Inter</b></div></div></div></div>; }
...
function Publish() { return <div className="section-card"><CardHead title="Publicación" text="Dos accesos separados: gestión privada para la constructora y experiencia pública para sus compradores."/><div className="publish-links"><PublishLink eyebrow="CLIENTE · PRIVADO" title="Control comercial" path={clientUrl} description="Precios · disponibilidad · métricas"/><PublishLink eyebrow="PÚBLICO · LECTOR" title="Reader / Embed" path={readerUrl} description="Experiencia 3D · solo lectura · para insertar en su web"/></div><div className="publish-url"><small>SHOWROOM DIRECTO</small><strong>/proyecto/{slug}</strong><a href={`/proyecto/${slug}`}>Abrir ↗</a></div><div className="checklist">{['Proyecto configurado','Inventario cargado','Branding aplicado','Experiencia configurada','Accesos generados'].map((x)=><div key={x}><span>✓</span>{x}<b>OK</b></div>)}</div><button className="publish-button">Publicar cambios</button></div>; }
