...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    platformApi.getProjectUnitsAdmin(selectedId).then(setUnits).catch(() => setUnits([]));
  }, [selectedId]);
  ...
  return <div className="admin-shell">
    <aside className="admin-sidebar">
      ...
      <div className="admin-side-bottom"><a href="/admin/structure">▦ Data Studio</a><a href="/studio">＋ Project Studio</a><a href={`/proyecto/${slug}`}>↗ Ver showroom</a></div>
    </aside>
...
function Plans({ units }) { const floors = [...new Set(units.map((u) => u.floor).filter((v) => v !== undefined))]; return <div className="section-card"><CardHead title="Planos e inventario espacial" text="Resumen de pisos detectados desde el inventario real."/><div className="plans-layout"><div className="plan-drawing"><div className="plan-outline"><i/><i/><i/><i/><i/><i/><i/><i/></div><span>{floors.length ? `${floors.length} PISOS DETECTADOS` : 'PLANO PENDIENTE'}</span></div><div><h3>Vinculación</h3><p>{units.length} unidades disponibles para vinculación espacial.</p><button className="secondary-button" disabled>Importar plano PDF</button><button className="secondary-button" disabled>Validar unidades</button></div></div><a className="data-studio-link" href="/admin/structure">Abrir Data Studio para planos y estructura →</a></div>; }
