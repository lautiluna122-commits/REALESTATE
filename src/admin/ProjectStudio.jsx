...
function buildPreliminaryInventory(importedPlans = []) {
  ...
  for (const plan of importedPlans) {
    ...
    for (const entity of plan.entities ?? []) {
      ...
      byId.set(id, { id, number, floor, surface: null, bedrooms: null, bathrooms: null, terrace: null, price: null, currency: 'USD', status: 'DRAFT', plan: plan.sourceName ?? null, modelRef: null, images: [], source: 'plan-import' });
    }
  }
  return [...byId.values()];
}
...
function kindLabel(kind) {
  return ({ buildingModel: 'Modelo 3D', plan: 'Plano', exteriorRender: 'Render exterior', interiorRender: 'Render interior', panorama360: 'Panorama 360°', video: 'Video' })[kind] ?? 'Contenido';
}
...
export default function ProjectStudio() {
  ...
  function removeFile(file) { setFiles((current) => current.filter((item) => item !== file)); }

  function processContent() {
    setProcessing(true);
    const nextPlans = files.filter((file) => classifyFile(file) === 'plan').map((file, index) => normalizePlanImport({ sourceName: file.name, page: index + 1, extractedText: file.name, provenance: { source: 'browser-upload', adapter: 'filename-signals-v1' } }));
    const nextAssets = files.map((file) => createProjectAsset({ projectId: slug, name: file.name, kind: classifyFile(file), path: '', source: 'upload', status: 'PENDING', mimeType: file.type, size: file.size, metadata: { originalName: file.name, extension: file.name.split('.').pop()?.toLowerCase() } }));
    window.setTimeout(() => { setPlans(nextPlans); setAssets(nextAssets); setProcessing(false); setStep(2); }, 500);
  }
  ...
  async function generate() {
    ...
    const project = {
      ...
      config: { source: 'project-studio', generation: 'showroom-manifest-v1', inventorySource: preliminaryInventory.length ? 'plan-import-preliminary' : 'manual-pending' },
    };
    ...
    try {
      ...
      let company = companies.find((item) => item.slug === companySlug);
      if (!company) company = await platformApi.createCompany({ name: developer || 'Desarrolladora', slug: companySlug });

      let serverProject;
      try {
        serverProject = await platformApi.createProject({ companyId: company.id, name: projectName, slug, description: '', status: 'DRAFT', location: { name: location }, branding: {}, environmentConfig: {}, publicationConfig: { publicSlug: slug, publicUrl: `/proyecto/${slug}` } });
      } catch {
    ...
    <main className="studio-shell">
      ...
      <section className="studio-card">
        ...
        {step === 1 && <>
          ...
          <label className={`dropzone ${dragging ? 'dragging' : ''}`} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files); }}><input type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.svg,.glb,.gltf,.fbx,.obj,.webp,.mp4,.webm,.mov" onChange={(event) => addFiles(event.target.files)} /><strong>Soltá los archivos del proyecto acá</strong><span>planos · 3D · renders · videos · 360°</span><small>PDF · PNG · JPG · SVG · GLB · GLTF · FBX · OBJ · WEBP · MP4 · WEBM</small></label>
          {files.length > 0 && <><div className="file-list">{files.map((file) => { const kind = classifyFile(file); return <div className="file-row" key={`${file.name}-${file.size}`}><div><b>{file.name}</b><small>{kindLabel(kind)} · {file.type || 'archivo'} · {formatSize(file.size)}</small></div><button aria-label={`Quitar ${file.name}`} onClick={() => removeFile(file)}>×</button></div>; })}</div><div className="review-note"><b>{files.length} archivos preparados</b><p>{counts.buildingModel ?? 0} modelo(s) 3D · {counts.plan ?? 0} plano(s) · {(counts.exteriorRender ?? 0) + (counts.interiorRender ?? 0)} render(s) · {counts.panorama360 ?? 0} panorama(s) · {counts.video ?? 0} video(s).</p></div></>}
