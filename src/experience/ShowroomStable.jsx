  ...
  return (
    <main className={`stableShowroom ${night ? 'isNight' : ''}`}>
      ...
      <section id="experiencia" className="stableSection experience">
        ...
        <div className="experiencePanel"><div className="experienceTabs">{['3D interactivo', 'Interiores', 'Amenities', 'Ubicación'].map((item, i) => <button key={item} className={activeExperience === item ? 'active' : ''} onClick={() => setActiveExperience(item)}><span>0{i + 1}</span>{item}</button>)}</div><div className="experienceDetail"><small>EXPERIENCIA / {activeExperience.toUpperCase()}</small><h3>{activeExperience === '3D interactivo' ? 'Recorré la arquitectura.' : activeExperience === 'Interiores' ? 'Entrá antes de visitar.' : activeExperience === 'Amenities' ? 'Descubrí cómo se vive.' : 'Entendé dónde estás comprando.'}</h3>{activeExperience === '3D interactivo' && <p>{remoteModelUrl ? 'Modelo 3D del proyecto cargado. Girá, acercate y seleccioná una unidad desde el inventario.' : 'Exploración 3D disponible. El modelo del proyecto aparecerá automáticamente cuando la constructora lo publique.'}</p>}{activeExperience === 'Interiores' && <p>{units.some((unit) => Array.isArray(unit.images) && unit.images.length > 0) ? 'Las imágenes interiores asociadas a las unidades aparecen en su ficha.' : 'Las imágenes, recorridos 360 y material interior se mostrarán cuando sean publicados para este proyecto.'}</p>}{activeExperience === 'Amenities' && <p>{project.amenities?.length ? project.amenities.length + ' espacios y servicios publicados para este proyecto.' : 'Los amenities publicados por la constructora aparecerán aquí.'}</p>}{activeExperience === 'Ubicación' && <p>{project.location?.coordinates ? (project.location?.city || '') + (project.location?.district ? ' · ' + project.location.district : '') + '. Coordenadas disponibles para integrar mapa y navegación.' : 'La ubicación y navegación del proyecto aparecerán cuando estén configuradas.'}</p>}</div></div>
      </section>
