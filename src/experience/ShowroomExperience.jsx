...
function Window({ x, y, z, sx = 1, sy = 1, night }) {
  return (
    <mesh position={[x, y, z]}>
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      {floors.map((floor) => {
        ...
        const isFocused = selectedFloor === floor;
        return (
          <group key={floor} position={[0, y, 0]}>
            <mesh position={[0, 0, 0]}>
              <boxGeometry args={[14.5, isFocused ? 0.22 : 0.14, 10]} />
              <meshStandardMaterial color={isFocused ? '#d5b46c' : '#b6b0a8'} roughness={0.72} metalness={isFocused ? 0.2 : 0} emissive={isFocused ? '#6d4f18' : '#000000'} emissiveIntensity={isFocused ? 0.28 : 0} />
            </mesh>
              ...
              return (
                <group key={unit.id} position={[x, 0.7, 4.58]}>
                  <mesh onClick={(event) => { event.stopPropagation(); onSelectUnit(unit); }} onPointerOver={() => { document.body.style.cursor = 'pointer'; }} onPointerOut={() => { document.body.style.cursor = 'default'; }} castShadow>
                    ...
                    <meshStandardMaterial color={statusColor(unit.status, selected)} emissive={selected ? '#7b5c1c' : isFocused ? '#4e4124' : '#000'} emissiveIntensity={selected ? 0.7 : isFocused ? 0.16 : 0.05} roughness={0.52} metalness={0.25} />
                  </mesh>
...
function UnitCard({ unit, onPlan, onClose, onContact }) {
  if (!unit) return null;
  return <aside className="unit-detail">
    ...
    <div className="unit-actions"><button className="btn-dark" onClick={onPlan}>Ver plano</button><button className="btn-outline" onClick={() => onContact(unit)}>Consultar</button></div>
  </aside>;
}
...
function ContactModal({ unit, onClose }) {
  if (!unit) return null;
  const message = `Hola, quiero consultar por la unidad ${unit.number} de Ocean Mansions.`;
  const whatsapp = `https://wa.me/?text=${encodeURIComponent(message)}`;
  return <div className="modal" onClick={onClose}><div className="contact-modal" onClick={(event) => event.stopPropagation()}>
    <div className="modal-head"><div><span>CONSULTA PRIVADA</span><h3>Unidad {unit.number}</h3></div><button className="icon-close" onClick={onClose}>×</button></div>
    <p className="contact-intro">Estás consultando por una unidad <strong>{unit.type}</strong> de {unit.area} m², disponible por {money.format(unit.price)}.</p>
    <div className="contact-summary"><span>Superficie <b>{unit.area} m²</b></span><span>Terraza <b>{unit.terrace} m²</b></span><span>Estado <b>{statusLabel(unit.status)}</b></span></div>
    <a className="contact-whatsapp" href={whatsapp} target="_blank" rel="noreferrer">Continuar por WhatsApp ↗</a>
  </div></div>;
}
...
export default function ShowroomExperience() {
  ...
  const [contactUnit, setContactUnit] = useState(null);
  ...
  return <div className="experience">
    ...
    <main>
      <section id="proyecto" className="hero">
        ...
        {selectedUnit && <UnitCard unit={selectedUnit} onPlan={() => setPlanUnit(selectedUnit)} onContact={setContactUnit} onClose={() => setSelectedUnit(null)} />}
      </section>
      ...
      <section className="final-cta"><span className="eyebrow">OCEAN MANSIONS</span><h2>La próxima visita<br/><em>puede empezar acá.</em></h2><button className="btn-light" onClick={() => setContactUnit(selectedUnit ?? units[0] ?? null)}>Agendar visita ↗</button></section>
    </main>
    {planUnit && <PlanModal unit={planUnit} onClose={() => setPlanUnit(null)} />}
    {contactUnit && <ContactModal unit={contactUnit} onClose={() => setContactUnit(null)} />}
  </div>;
}
