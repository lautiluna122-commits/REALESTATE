...
const floors = [...new Set(units.map((unit) => unit.floor).filter((floor) => floor != null))].sort((a, b) => a - b);
...
function statusLabel(status) { return STATUS_LABELS[status] ?? status ?? 'Sin estado'; }
...
function PoolArea({ night }) {
  return <group position={[0, 0, 12]}>
    ...
    {[-9.5, 9.5].map((x) => <group key={x} position={[x, .55, 0]}>{[-2.4, 0, 2.4].map((z) => <RoundedBox key={z} args={[2.2, .22, .8]} radius={.12} smoothness={3} position={[0, 0, z]}><meshStandardMaterial color="#e9e1d3" roughness={.75} /></RoundedBox>)}</group>)}
  </group>;
}
...
function FacadeUnit({ unit, x, y, z, selected, night, onSelect }) {
  ...
  return <group position={[x, y, z]}>
    <RoundedBox args={[2.95, 2.05, .5]} radius={.08} smoothness={3}><meshStandardMaterial color={selected ? '#e4d4b1' : '#d8d2c8'} roughness={.62} metalness={.15} /></RoundedBox>
    <mesh position={[0, .12, .31]} onClick={(event) => { event.stopPropagation(); onSelect(unit); }} onPointerOver={() => { document.body.style.cursor = 'pointer'; }} onPointerOut={() => { document.body.style.cursor = 'default'; }}><boxGeometry args={[2.45, 1.52, .12]} /><meshStandardMaterial color={night ? COLORS.glassNight : COLORS.glass} roughness={.1} metalness={.65} emissive={night ? '#8f5d20' : color} emissiveIntensity={selected ? .85 : night ? .35 : .06} /></mesh>
...
function Tower({ selectedFloor, selectedUnit, night, onSelectUnit }) {
  ...
  return <group>
    ...
    {Array.from({ length: maxFloor }).map((_, index) => {
      ...
      const y = podiumTop + index * floorHeight + floorHeight / 2;
      const floorUnits = units.filter((unit) => unit.floor === floor);
      const focused = selectedFloor == null || selectedFloor === floor;
      const opacity = focused ? 1 : .10;
      return <group key={floor} position={[0, y, 0]}>
        <mesh position={[0, -floorHeight / 2, 0]} receiveShadow><boxGeometry args={[17.8, .16, 12.1]} /><meshStandardMaterial color={selectedFloor === floor ? '#c9ae78' : '#b8b0a5'} roughness={.72} transparent opacity={opacity} /></mesh>
        {selectedFloor === floor && <mesh position={[0, -.28, 6.08]}><boxGeometry args={[15.8, .08, .08]} /><meshStandardMaterial color="#e4c26f" emissive="#8a641e" emissiveIntensity={1.4} metalness={.55} roughness={.22} /></mesh>}
        <mesh><boxGeometry args={[.48, floorHeight, 11.3]} /><meshStandardMaterial color={COLORS.concrete} roughness={.65} transparent opacity={opacity} /></mesh>
        ...
        {Array.from({ length: 5 }).map((_, i) => <mesh key={i} position={[-6.5 + i * 3.25, 0, -5.72]}><boxGeometry args={[2.35, 1.75, .1]} /><meshStandardMaterial color={night ? '#c5a96e' : '#789aa2'} roughness={.16} metalness={.55} emissive={night ? '#9a6728' : '#193d46'} emissiveIntensity={night ? .38 : .04} transparent opacity={opacity} /></mesh>)}
      </group>;
    })}
...
function Interior({ unit, night }) {
  return <group position={[0, -1.2, 0]}>
    ...
    <RoundedBox args={[7.2, .8, 3.1]} radius={.25} smoothness={4} position={[-4.3, 1, 1.8]}><meshStandardMaterial color="#a99b88" roughness={.8} /></RoundedBox>
    ...
    <Html position={[-8.7, 5.8, -5.9]} transform distanceFactor={10}><div className="interior-label"><span>UNIDAD {unit?.number ?? '—'}</span><strong>{unit?.type ?? 'Interior'}</strong><small>{unit?.area ?? unit?.surface ?? 0} m² · {unit?.bedrooms ?? 0} dormitorios</small></div></Html>
  </group>;
}
...
function CameraRig({ mode, controlsRef, selectedFloor }) {
  ...
  useFrame(({ camera }) => {
    ...
    const lift = selectedFloor && mode !== 'interior' ? (selectedFloor - 1) * .32 : 0;
    camera.position.lerp(new Vector3(preset.position.x, preset.position.y + lift, preset.position.z), .045);
    controlsRef.current?.target.lerp(new Vector3(preset.target.x, preset.target.y + lift, preset.target.z), .055);
  });
  return null;
}
...
function Scene({ selectedFloor, selectedUnit, night, cameraMode, tour, interior, onSelectUnit }) {
  ...
  return <Canvas shadows camera={{ position: [34, 24, 40], fov: 35 }} dpr={[1, 1.7]} gl={{ antialias: true, powerPreference: 'high-performance' }}>
    <color attach="background" args={[interior ? (night ? '#17120e' : '#ddd5c8') : (night ? '#071219' : '#a8c6cf')]} />
    <fog attach="fog" args={[interior ? '#d9d0c1' : (night ? '#071219' : '#a8c6cf'), interior ? 34 : 42, interior ? 65 : 100]} />
    {interior ? <><ambientLight intensity={night ? .52 : 1.35} /><pointLight position={[0, 5, 2]} intensity={night ? 1.4 : .8} color={night ? '#f0bf78' : '#fff4df'} /><Interior unit={selectedUnit ?? units[0]} night={night} /></> : <><ambientLight intensity={night ? .42 : 1.18} /><directionalLight castShadow position={[18, 32, 10]} intensity={night ? 1.25 : 3.15} color={night ? '#e5bd75' : '#fff1d0'} shadow-mapSize-width={2048} shadow-mapSize-height={2048} /><Environment preset="city" /><Sky distance={450000} sunPosition={night ? [-4, -2, 2] : [8, 8, 5]} inclination={night ? .82 : .48} azimuth={.22} /><Site night={night} /><Tower selectedFloor={selectedFloor} selectedUnit={selectedUnit} night={night} onSelectUnit={onSelectUnit} /><ContactShadows position={[0, -.02, 0]} opacity={night ? .3 : .42} scale={70} blur={2.5} far={45} /></>}
...
function UnitCard({ unit, onPlan, onClose, onInterior }) {
  if (!unit) return null;
  const area = unit.area ?? unit.surface ?? 0;
  return <aside className="unit-detail"><button className="icon-close" onClick={onClose}>×</button><div className="unit-kicker">UNIDAD {unit.number} · PISO {unit.floor}</div><h2>{unit.type ?? 'Unidad'}</h2><div className={`availability ${(unit.status ?? 'HIDDEN').toLowerCase()}`}>{statusLabel(unit.status)}</div><div className="unit-price">{money.format(unit.price ?? 0)}</div><div className="unit-metrics"><span><b>{area} m²</b> superficie</span><span><b>{unit.terrace ?? 0} m²</b> terraza</span><span><b>{unit.bedrooms ?? 0}</b> dormitorios</span><span><b>{unit.bathrooms ?? 0}</b> baños</span></div><p>{unit.description ?? 'Información comercial del proyecto.'}</p><div className="unit-actions"><button className="btn-dark" onClick={onInterior}>Entrar a la unidad <span>↗</span></button><button className="btn-outline" onClick={onPlan}>Ver plano</button></div></aside>;
}

function PlanModal({ unit, onClose }) {
  if (!unit) return null;
  const area = unit.area ?? unit.surface ?? 0;
  return <div className="modal" onClick={onClose}><div className="plan-modal" onClick={(event) => event.stopPropagation()}><div className="modal-head"><div><span>PLANO · UNIDAD {unit.number}</span><h3>{unit.type ?? 'Unidad'} · {area} m²</h3></div><button className="icon-close" onClick={onClose}>×</button></div><div className="plan-tabs"><button className="active">2D</button><button>3D</button><button>AMOBLADO</button></div><svg viewBox="0 0 820 480" className="plan-art"><rect x="20" y="20" width="780" height="440" rx="22" fill="#f5efe4" stroke="#b8aa96" strokeWidth="3"/><rect x="50" y="50" width="300" height="165" fill="#d9e6e8" stroke="#a7b7b9" strokeWidth="3"/><rect x="370" y="50" width="390" height="115" fill="#eadbc5" stroke="#c8b18f" strokeWidth="3"/><rect x="50" y="235" width="205" height="190" fill="#e1ddd5" stroke="#b9afa1" strokeWidth="3"/><rect x="275" y="235" width="205" height="190" fill="#dce6da" stroke="#aebdac" strokeWidth="3"/><rect x="500" y="190" width="260" height="235" fill="#d8e0e5" stroke="#a8b5be" strokeWidth="3"/><text x="128" y="140" fontSize="25" fill="#23313a">Living / Comedor</text><text x="510" y="118" fontSize="24" fill="#4b4032">Cocina</text><text x="105" y="330" fontSize="20" fill="#3b3935">Dormitorio</text><text x="330" y="330" fontSize="20" fill="#3b3935">Dormitorio</text><text x="575" y="285" fontSize="20" fill="#3b3935">Suite</text><text x="590" y="350" fontSize="20" fill="#3b3935">Baños</text><text x="325" y="448" fontSize="18" fill="#6b6258">Terraza · {unit.terrace ?? 0} m²</text></svg></div></div>;
}

function ContactModal({ unit, onClose }) {
  const name = project.name ?? 'Proyecto';
  return <div className="modal" onClick={onClose}><div className="contact-modal" onClick={(event) => event.stopPropagation()}><button className="icon-close" onClick={onClose}>×</button><span className="eyebrow">SOLICITAR INFORMACIÓN</span><h3>{unit ? `Unidad ${unit.number}` : name}</h3><p>{unit ? `Quiero recibir información sobre la unidad ${unit.number}, ${unit.type ?? 'unidad'}, piso ${unit.floor}.` : `Quiero recibir información sobre ${name}.`}</p><div className="contact-actions"><button className="btn-dark" onClick={() => window.open(`https://wa.me/?text=${encodeURIComponent(unit ? `Hola, quiero información sobre la unidad ${unit.number} de ${name}.` : `Hola, quiero información sobre ${name}.`)}`, '_blank')}>Continuar por WhatsApp ↗</button><button className="btn-outline" onClick={onClose}>Seguir recorriendo</button></div></div></div>;
}
...
export default function ShowroomExperience() {
  ...
  const types = [...new Set(units.map((unit) => unit.type).filter(Boolean))]; const fromPrice = units.length ? Math.min(...units.map((unit) => unit.price ?? 0)) : 0; const available = units.filter((unit) => unit.status === 'AVAILABLE').length; const reserved = units.filter((unit) => unit.status === 'RESERVED').length; const sold = units.filter((unit) => unit.status === 'SOLD').length;
  const go = (id) => { setActiveSection(id); document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' }); };
  const focusUnit = (unit) => { setSelectedUnit(unit); setSelectedFloor(unit.floor); setCameraMode('close'); setInterior(false); go('proyecto'); };
  ...
  useEffect(() => { if (!tour || interior) return undefined; const sequence = ['master', 'approach', 'building', 'pool', 'roof', 'building']; const timer = window.setInterval(() => { setTourStep((step) => { const next = (step + 1) % sequence.length; setCameraMode(sequence[next]); return next; }); }, 5200); return () => window.clearInterval(timer); }, [tour, interior]);
  ...
  return <div className={`experience ${night ? 'is-night' : ''} ${interior ? 'is-interior' : ''}`}>
    ...
    <main>
      <section id="proyecto" className="hero"><div className="scene"><Scene selectedFloor={selectedFloor} selectedUnit={selectedUnit} night={night} cameraMode={cameraMode} tour={tour} interior={interior} onSelectUnit={(unit) => { setSelectedUnit(unit); setSelectedFloor(unit.floor); setCameraMode('close'); setInterior(false); }} /></div><div className="hero-gradient" /><div className="cinematic-vignette" />
        {!interior ? <><div className="hero-copy"><span className="eyebrow">{project.location?.district?.toUpperCase() ?? 'PROYECTO INMOBILIARIO'} · {project.location?.city?.toUpperCase() ?? 'URUGUAY'}</span><h1>{project.name?.split(' ').slice(0, -1).join(' ') || project.name}<br/><em>{project.name?.split(' ').slice(-1).join(' ')}</em></h1><p>Recorré el proyecto. Explorá cada piso. Encontrá la unidad que querés.</p><div className="hero-cta"><button className="btn-dark" onClick={() => { setCameraMode('building'); setSelectedFloor(null); }}>Explorar edificio <span>↗</span></button><button className="btn-ghost" onClick={() => setTour((value) => !value)}>{tour ? 'Detener tour' : 'Tour cinematográfico'}</button></div></div>
          ...
          <div className="hero-stats"><span><b>{units.length}</b> unidades</span><span><b>{floors.length}</b> pisos</span><span><b>USD {Math.round(fromPrice / 1000)}K</b> desde</span></div><div className="scene-meta"><span>SCENE 0{tourStep + 1}</span><i /><span>{tour ? 'CINEMATIC TOUR' : 'INTERACTIVE SHOWROOM'}</span></div>
          {selectedUnit && <UnitCard unit={selectedUnit} onPlan={() => setPlanUnit(selectedUnit)} onClose={() => setSelectedUnit(null)} onInterior={() => enterInterior(selectedUnit)} />}<div className="scene-hint"><span>DRAG · ORBITAR</span><span>SCROLL · ZOOM</span><span>CLICK · EXPLORAR</span></div>
        </> : <div className="interior-overlay"><div><span className="eyebrow">RECORRIENDO UNIDAD {selectedUnit?.number}</span><h2>{selectedUnit?.type ?? 'Unidad'}</h2><p>{selectedUnit?.area ?? selectedUnit?.surface ?? 0} m² · {selectedUnit?.bedrooms ?? 0} dormitorios · piso {selectedUnit?.floor ?? '—'}</p></div><button className="btn-light" onClick={() => { setInterior(false); setCameraMode('close'); }}>← Volver al edificio</button></div>}
      </section>
      {!interior && <><section className="intro-band"><div><span className="eyebrow">EL PROYECTO</span><h2>Una propiedad<br/><em>para recorrer.</em></h2></div><p>La experiencia conecta arquitectura, disponibilidad e información comercial en un mismo recorrido.</p><div className="intro-price"><span>Unidades desde</span><strong>{money.format(fromPrice)}</strong></div></section>
      <section id="unidades" className="section units-section"><div className="section-head"><div><span className="eyebrow">INVENTARIO EN TIEMPO REAL</span><h2>Encontrá tu<br/><em>unidad.</em></h2></div><p>Seleccioná una tipología o un piso. Cada unidad queda vinculada al edificio 3D.</p></div><div className="finder"><select value={finderType} onChange={(e) => setFinderType(e.target.value)}><option>Todos</option>{types.map((type) => <option key={type}>{type}</option>)}</select><select value={finderFloor} onChange={(e) => setFinderFloor(e.target.value)}><option>Todos</option>{floors.map((floor) => <option key={floor}>{floor}</option>)}</select><div className="legend"><span><i className="dot available" /> {available} disponibles</span><span><i className="dot reserved" /> {reserved} reservadas</span><span><i className="dot sold" /> {sold} vendidas</span></div><span className="result-count">{finderUnits.length} resultados</span></div><div className="unit-grid">{finderUnits.map((unit) => <button key={unit.id} className="unit-tile" onClick={() => focusUnit(unit)}><div className="tile-top"><strong>{unit.number}</strong><i className={`dot ${(unit.status ?? 'HIDDEN').toLowerCase()}`} /></div><span>{unit.type ?? 'Unidad'} · Piso {unit.floor}</span><b>{money.format(unit.price ?? 0)}</b><small>{unit.area ?? unit.surface ?? 0} m² · {unit.bedrooms ?? 0} dorm.</small></button>)}</div></section>
      <section id="amenities" className="section dark-section"><div className="section-head light"><div><span className="eyebrow">AMENITIES</span><h2>Todo lo que<br/><em>te rodea.</em></h2></div><p>Espacios pensados para vivir el edificio como un destino.</p></div><div className="amenity-grid">{project.amenities?.map((amenity, index) => <article key={amenity.id}><span className="amenity-number">0{index + 1}</span><h3>{amenity.name}</h3><p>{amenity.description}</p><button onClick={() => { setCameraMode(index === 0 ? 'pool' : 'building'); go('proyecto'); }}>Explorar espacio ↗</button></article>)}</div></section>
      <section id="ubicacion" className="location-section"><div className="location-copy"><span className="eyebrow">UBICACIÓN</span><h2>{project.location?.district?.split(' ')[0] ?? 'Tu'}<br/><em>{project.location?.district?.split(' ').slice(1).join(' ') || project.location?.city || 'Ubicación'}.</em></h2><p>Una ubicación conectada con el entorno y los puntos que hacen atractivo al proyecto.</p><button className="btn-dark" onClick={() => { const coordinates = project.location?.coordinates ?? { lat: -34.9, lng: -54.9 }; window.open(`https://www.google.com/maps/search/?api=1&query=${coordinates.lat},${coordinates.lng}`, '_blank'); }}>Abrir en Maps ↗</button></div><div className="map-card"><div className="map-grid" /><span className="map-pin main">{project.name}</span><div className="map-distance"><strong>{project.location?.city ?? 'Ubicación'}</strong><br/>{project.location?.district ?? ''}</div></div></section><section className="final-cta"><span className="eyebrow">TU PRÓXIMA PROPIEDAD</span><h2>¿La<br/><em>recorremos?</em></h2><button className="btn-light" onClick={() => setContactOpen(true)}>Quiero información ↗</button></section></>}
