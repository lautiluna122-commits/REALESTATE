...
const units = [
  { id: '804', floor: 8, type: '3 dormitorios', area: 142, price: 890000, status: 'Disponible' },
  { id: '805', floor: 8, type: '2 dormitorios', area: 118, price: 735000, status: 'Reservado' },
  { id: '704', floor: 7, type: '3 dormitorios', area: 142, price: 860000, status: 'Disponible' },
  { id: '604', floor: 6, type: '3 dormitorios', area: 142, price: 825000, status: 'Vendida' },
];

const floors = Array.from({ length: 12 }, (_, index) => 12 - index);
...
export default function DemoPortal() {
  ...
  const filteredUnits = useMemo(() => units.filter((unit) => {
    return (floor === 'Todos' || unit.floor === Number(floor)) && (status === 'Todos' || unit.status === status);
  }), [floor, status]);
  ...
  return (
    <div className="demo-portal">
      ...
      <main className="demo-main">
        ...
        {section === 'plans' && <EmptyPanel title="Planos" text="Acá vivirá la biblioteca de planos por edificio, piso y unidad." />}
        {section === 'amenities' && <EmptyPanel title="Amenities" text="Piscina, wellness, sky lounge, lobby y futuras experiencias interactivas." />}
        {section === 'publication' && <Publication onShowroom={goShowroom} />}
      </main>
    </div>
  );
}
