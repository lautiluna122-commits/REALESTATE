...
async function q(table: string, params: any = {}) { const order = String(params.order || "createdat.asc"); const [column,direction] = order.split("."); const { data, error } = await db.from(table).select(params.select || "*").match(params.match || {}).order(column || "createdat", { ascending: direction !== "desc" }); if (error) throw error; return data || []; }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (path[0] === "admin" && path[1] === "companies" && method === "POST") {
    ...
    if (path[0] === "public") {
      if (path[1] === "projects" && path[2]) {
        ...
        const [buildings, floors, units, plans, amenities, assets, locations, experience] = await Promise.all([q("buildings", { match: { projectid: pid } }), q("floors", { match: { projectid: pid } }), q("units", { match: { projectid: pid } }), q("plans", { match: { projectid: pid } }), q("amenities", { match: { projectid: pid } }), q("assets", { match: { projectid: pid } }), q("locations", { match: { projectid: pid } }), q("project_experience_configs", { match: { projectid: pid } })]);
        return json({ project: publicProject(rawProject), publication: pub(publication), buildings, floors, units: units.map(unit), plans, amenities, assets, location: locations[0] || null, experience: experience[0] || null });
      }
    ...
    if (path[0] === "admin" || path[0] === "company") {
      ...
      if (path[1] === "projects" && !path[2] && method === "GET") return json((await q("projects", { match: { companyid: c.id }, order: "createdat.desc" })).map(project));
      ...
      if (path[1] === "projects" && path[2]) {
        ...
        if (resource === "buildings") {
          ...
          if (method === "DELETE" && rest[1]) {
            ...
            if (existingError) throw existingError; if (!existing) return json({ message: "Building not found" }, 404);
            const { error: unitsError } = await db.from("units").delete().eq("buildingid", buildingId).eq("projectid", pid); if (unitsError) throw unitsError;
            const { error: floorsError } = await db.from("floors").delete().eq("buildingid", buildingId).eq("projectid", pid); if (floorsError) throw floorsError;
            const { error: deleteError } = await db.from("buildings").delete().eq("id", buildingId).eq("projectid", pid); if (deleteError) throw deleteError;
            return json({ deleted: true, id: buildingId });
          }
          if (method !== "POST") return json({ message: "Method not allowed" }, 405);
          const b = await parse(req); if (!String(b.name || "").trim()) return json({ message: "building name is required" }, 400);
          const { data, error } = await db.from("buildings").insert({ id: id(), projectid: pid, name: String(b.name).trim(), reference: String(b.reference || ""), metadata: b.metadata || null, createdat: now() }).select().single(); if (error) throw error; return json(data, 201);
        }
        if (resource === "floors") {
        ...
        if (resource === "experience") { if (method === "GET") { const { data, error } = await db.from("project_experience_configs").select("*").eq("projectid", pid).maybeSingle(); if (error) throw error; return json(data || null); } if (method === "PUT") { const body = await parse(req); const existing = await db.from("project_experience_configs").select("id").eq("projectid", pid).maybeSingle(); const payload = { projectid: pid, schema_version: Number(body.schemaVersion || 1), config: body.config || {}, generated_by: body.generatedBy || "manual", approved: Boolean(body.approved), updatedat: now() }; const result = existing.data ? await db.from("project_experience_configs").update(payload).eq("projectid", pid).select().single() : await db.from("project_experience_configs").insert({ id: id(), createdat: now(), ...payload }).select().single(); if (result.error) throw result.error; return json(result.data); } }
      }
    }
